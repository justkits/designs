...
| `offset`            | `number`                                 | `16`       | 툴팁과 뷰포트 가장자리 사이의 최소 간격 (픽셀 단위)                           |
...
| `disabled`          | `boolean`                                | `false`    | `true`이면 툴팁을 열지 않으며, `Tooltip.Trigger`의 버튼도 비활성화된다.       |

### `Tooltip.Trigger`

Extends `ButtonHTMLAttributes<HTMLButtonElement>`. (`aria-describedby`, `onMouseEnter`, `onMouseLeave`, `onFocus`, `onBlur`, `onTouchStart`, `onTouchEnd`, `onTouchMove`, `onTouchCancel` 제외)

- `type`: 기본값은 `"button"`. `asChild` 사용 시 자식 엘리먼트에서 override할 수 있다.
